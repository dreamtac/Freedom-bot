...
import { normalizeStockName, StockLookupError } from '../sources/domestic-stocks.js';
...
export const priceAlertCommand: BotCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('목록').setDescription('현재 실시간 주가 알림 종목을 확인합니다.'),
        ),
    ...
    async autocomplete(interaction, context) {
        ...
        const storedOverseas =
            ...
            typeof context.stockStore.suggestOverseas === 'function'
                ...
                : [];
        const overseas =
            storedOverseas.length > 0 ? storedOverseas : suggestOverseasStocks(focusedValue, 25 - domestic.length);
        await interaction.respond([
    ...
    async execute(interaction, context) {
        ...
        if (subcommand === '목록') {
            await interaction.reply({
                embeds: [
                    buildPriceAlertListEmbed(
                        context.priceAlertStore.list(),
                        context.priceAlertStore.getMaximumStockAlerts(),
                    ),
                ],
                flags: MessageFlags.Ephemeral,
            });
        ...
        if (subcommand === '추가') {
            ...
            try {
                ...
                await interaction.reply({
                    content: error instanceof Error ? error.message : '주가 알림 종목을 추가하지 못했습니다.',
                    flags: MessageFlags.Ephemeral,
                });
            }
            return;
...
async function resolveStock(
    ...
    try {
        if (stockStore) {
            ...
            if ('countOverseas' in stockStore && stockStore.countOverseas() > 0) {
                try {
        ...
        return error instanceof Error ? error : new StockLookupError('종목을 찾지 못했습니다.');
    }
}

function buildPriceAlertListEmbed(
