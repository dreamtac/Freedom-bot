...
- [x] `er_games`에 게임 결과 숫자 열과 JSON 열을 추가한다.
- [x] `er_game_receipts` 테이블과 `(channel_id, game_id)` 고유 키, 상태 인덱스를 추가한다.
- [x] `er_game_receipt_players` 테이블에 게임 결과별 등록 친구·팀 번호를 저장한다.
- [x] 유저별 `receipt_enabled`와 선택적 채널 설정을 추가한다.
- [x] 기존 데이터가 있는 DB에서 반복 마이그레이션해도 안전하게 한다.
- [x] 회전한 API `userId`를 통합할 때 선수 연결은 이전하되 경기 단위 게임 결과 상태는 유지한다.
- [x] 기존 100경기를 `suppressed` 또는 기준선으로 초기화해 과거 알림을 막는다.
- [x] 대기·성공·실패·재시작 복구와 중복 방지를 테스트한다.

결과: 게임 결과 계산용 숫자 필드와 원본 JSON을 `er_games`에 보존하고, 채널과 경기의 조합을 유일 키로 갖는 발송 큐를 추가했다. 같은 경기에서 감지된 등록 친구는 팀 번호와 함께 하나의 게임 결과에 합쳐진다. `pending → sending → sent/failed` 상태와 시도 횟수·오류·Discord 메시지 ID를 기록하며, 시작 시 중단된 `sending` 항목은 다시 `pending`으로 복구한다. 최초 도입 시 이미 수집된 경기는 `suppressed` 기준선으로 초기화할 수 있다. UID가 회전하면 선수 연결과 유저별 알림 설정만 새 UID로 옮기고 경기 단위 발송 상태는 그대로 유지한다. 기존 스키마에서 열과 테이블을 제거한 뒤 두 차례 다시 여는 마이그레이션, 중복 등록, 실패 재시도, 재시작 복구, 발송 완료 후 재등록 방지, 기준선 반복 실행을 테스트했다.

완료 기준: 재시작과 UID 회전 뒤에도 경기당 게임 결과가 한 번만 전송 대상이 된다.

### 3. 최신 수집과 게임 결과 큐 연결

- [x] 수집 결과에 `newGameIds`를 추가하고 한 자동 갱신 주기가 끝날 때까지 누적한다.
- [x] 최신 수집과 백필의 새 행을 구분한다. 백필은 결과 알림을 만들지 않는다.
- [x] 최신 페이지가 여러 장일 때 모든 새 경기 ID를 빠짐없이 반환한다.
- [x] 주기 종료 후 `gameId`로 중복 제거하고 DB 저장과 `pending` 게임 결과 생성을 안전한 트랜잭션 경계로 묶는다.
- [x] 같은 경기에서 감지된 등록 친구를 한 게임 결과의 선수 목록으로 합친다.
- [x] 같은 자동 주기가 중복 실행돼도 대기 게임 결과가 중복 생성되지 않게 한다.
- [x] 한 유저 실패가 다른 유저의 수집·게임 결과 전송을 막지 않게 한다.
- [x] 새 경기 0개, 1개, 10개 초과, 중간 API 실패, 재시작을 테스트한다.
- [x] 등록 친구 3명이 같은 `gameId`를 반환하면 대기 결과와 Discord 전송이 각각 1개만 생기는지 테스트한다.
- [x] 같은 경기가 다음 주기에 다시 보일 때 재발송되지 않는지 테스트한다.
- [x] 같은 게임의 등록 친구가 서로 다른 팀이면 한 게임 결과 안에서 팀별로 나뉘는지 테스트한다.

결과: 최신 수집 결과에 완료 경계 이후의 `newGameIds`를 여러 페이지에 걸쳐 누적하고, 첫 수집과 백필은 빈 목록을 반환한다. 새 경기 행에는 `receipt_eligible` 표식을 경기 저장과 같은 트랜잭션에서 기록한다. 자동 주기 마지막에는 이 표식 중 아직 결과 선수와 연결되지 않은 경기만 읽어 채널·`gameId`별로 묶고, 전체 묶음을 한 트랜잭션으로 `pending` 큐에 넣는다. 따라서 페이지 저장 뒤 API 오류나 프로세스 재시작, 큐 생성 실패가 발생해도 다음 주기에 후보가 다시 발견된다. 세 등록 유저가 같은 경기에 있거나 서로 다른 팀이어도 결과는 한 건이고 선수별 팀 번호가 유지되며, 별도 솔로 경기는 별도 결과가 된다. 한 유저의 수집 실패는 다른 유저의 후보 큐 생성을 막지 않는다.

완료 기준: 자동 최신 수집에서 새 경기만 정확히 게임 결과 큐에 들어간다.

### 4. 계산과 기본 게임 결과 포맷

- [x] 전투·팀 기여·경제·행동·빌드 View Model을 만든다.
- [x] 총 크레딧과 상세 출처를 중복 합산하지 않는다.
- [x] 획득 크레딧을 시간·모드별 페이즈 보정·일반 동물·돌연변이·보스·킬·어시스트·현상금·최저 크레딧 보정·LUMI 도난 경보·보리·판매/교환·특성으로 나누고, 이름을 붙일 수 없는 잔여값만 `미분류`로 표시한다.
- [x] `crGetByGuideRobot`은 LUMI 수익으로, `damageToGuideRobot`은 LUMI 대상 피해로 구분하고 홀로그램 개수는 역산하지 않는다.
- [x] 네 종류의 `Bori*` 키를 보리 수익으로 합산하고 상세 출처와 `getBoriReward` 상자 기록을 별도로 표시한다.
- [x] 보리 피해량은 API에 없으므로 표시하지 않고 `damageToGuideRobot`을 보리 피해로 전용하지 않는다.
- [x] 활동 기록에 균열 진입·승리, 큐브 수집, 보리 보상 상자, 환경 변수·가젯, 시즌 기믹을 추가하되 0인 항목은 숨기고 기본 화면을 최대 5줄로 제한한다.
- [x] 선택한 전술 스킬 이름과 `tacticalSkillUseCount`를 활동 기록에 표시한다. 캐릭터 스킬과 무기 스킬 사용 횟수는 제공되지 않으므로 표시하거나 역산하지 않는다.
- [x] 빌드에 `skillOrderInfo`를 실제 스킬 레벨업 순서로 표시하고, 추천 루트의 `skillPath` 및 스킬 사용 횟수와 구분한다.
- [x] `routeIdOfStart`로 루트 이름과 현재 추천 수를 조회해 빌드에 표시한다. 추천 수는 조회 시점 값임을 밝히고, 조회 실패 시 루트 번호만 표시한다.
- [x] 루트 메타데이터를 ID별로 24시간 캐시하고 새 루트 조회만 공통 API 요청 큐를 사용한다.
- [x] `crGetAnimal`은 비변이 PvE 상위 집계값으로만 검증하고 상세 합계에는 쓰지 않는다. 일반 야생동물은 서로 겹치지 않는 `creditSource.Kill*` 키로 계산한다.
- [x] `crGetKill`과 `killItemBountyGainVFCredit`은 각각 기본 킬 보상과 현상금으로 분리한다.
- [x] 모든 경기에서 `상세 획득 합 + 미분류 = totalGainVFCredit`, `상세 사용 합 + 미분류 = totalUseVFCredit`을 검증하고 음수 잔여값이 생기면 전송하지 않는다.
- [x] 플레이어 피해 유형을 기본 공격·스킬·아이템 효과·직접 피해·고유 효과·함정으로 나누고 0과 결측 항목은 숨긴다.
- [x] 총피해와 피해 유형 합계를 실제 응답에서 대조하고 차이를 임의의 유형에 합산하지 않는다.
- [x] `ccTimeToPlayer`를 `가한 CC 시간`으로 표시하고 하드·소프트 CC 구분은 만들지 않는다.
- [x] `damageOffsetedByShield_Player`는 본인이 보호막으로 흡수한 대인 피해로 표시하고, 대상이 불명확한 `protectAbsorb`는 아군 보호량으로 표시하지 않는다.
- [x] 팀 기여를 지원과 시야 지원으로 나누고 아군 회복·부활·아군 원격 드론·시야 점수·카메라·보안 콘솔·정찰/EMP 드론을 표시한다.
- [x] 팀 기여에 옮긴 시야 관련 수치를 행동 화면에 중복 표시하지 않는다.
- [x] `TraitSkillCoinToss`를 코인 토스로 표시하고 총수입 대비 비율을 계산한다.
- [x] 할인 쿠폰 선택 시 재료별 실제 결제액을 표시한다. 절약액은 품목·패치별 정상 가격과 결제액이 정확히 대조될 때만 계산하고, 그렇지 않으면 추정값을 표시하지 않는다.
- [x] 전술 스킬 강화 모듈은 `tacticalSkillUpgradeUseVFCredit`, `crUseActivationModule`, `creditSource.TacticalSkillUpgrade`를 교차 검증해 실제 결제액을 표시한다. 현재 정상 가격은 200이며 할인 쿠폰 적용 표본은 180이다.
- [x] `itemTransferredDrone`의 아이템 코드를 이름과 수량으로 묶어 원격 드론 구매 내역에 표시한다. 총비용은 본인·아군용으로 구분하되 개별 품목 가격이나 수령자를 역산하지 않는다.
- [x] 감시 카메라·망원 카메라 설치와 정찰·EMP 드론 사용 횟수는 구매 내역과 분리해 팀 기여에 표시한다.
- [x] 0과 결측 항목을 숨기며 숫자·시간·퍼센트를 한국어 형식으로 표시한다.
- [x] 특징 후보를 우선순위에 따라 최대 4개 선정한다.
- [x] 등록 친구가 2명 이상인 경기의 팀 합계와 선수별 요약을 만든다.
- [x] 개인 최고 판정은 현재 경기 저장 전 기준 또는 현재 경기 제외 쿼리로 계산한다.
- [x] Embed 제목·설명·필드·전체 길이 제한을 적용한다.
- [x] 좁은 모바일 화면, 긴 실험체·아이템 이름, 특징 없음, RP 누락을 검증한다.

결과: 저장된 경기와 결과 선수 연결을 읽어 전투·팀 기여·크레딧·활동·빌드 View Model을 구성하는 빌더와 Discord 기본·상세 Embed 포맷을 추가했다. 크레딧은 API 총액을 기준으로 독립 출처만 한 번씩 분류하고 남는 값만 `미분류`로 표시한다. 상세 합계가 총액보다 크거나 재료·전술 스킬 강화의 교차 검증 값이 다르면 결과를 전송 가능한 상태로 만들지 않는다. 피해 유형, 본인 보호막 흡수, 가한 CC, 시야 지원, LUMI·보리·시즌 활동, 코인 토스와 할인 쿠폰 실제 결제액, 원격 드론 품목, 스킬 레벨업 순서를 각각 의미가 확인된 범위에서 표시한다. 루트 제목과 현재 추천 수는 공통 요청 큐를 사용하는 단일 조회 API로 가져와 DB에 24시간 캐시하며 실패하면 루트 번호만 유지한다. 개인과 여러 팀의 등록 친구 요약, 개인 최고 기록, 긴 이름과 Discord 길이 제한을 테스트했다.

완료 기준: 기본 게임 결과만 봐도 결과와 이번 경기의 특징을 10초 안에 파악할 수 있다.

### 5. Discord 발송과 상세 버튼

- [x] 전용 `EternalReturnReceiptMonitor` 또는 발송 서비스를 만든다.
- [x] `ER_RECEIPTS_ENABLED`와 채널 설정이 있을 때만 초기화한다.
- [x] `pending` 결과 알림을 오래된 순서로 전송하고 성공·실패 상태를 저장한다.
- [x] 실패는 제한된 횟수로 재시도하고, 반복 실패가 5분마다 무한 중복 로그를 만들지 않게 한다.
- [x] 기본 게임 결과에 전투·팀 기여·크레딧·행동·빌드 버튼을 추가한다.
- [x] 팀 결과 화면에는 상세 유저 선택 메뉴를 추가하고 선택 상태를 사용자별 임시 응답에 적용한다.
- [x] 버튼은 짧은 `receiptId`만 포함하고 API 토큰·닉네임을 `customId`에 넣지 않는다.
- [x] 버튼 응답은 공개 메시지를 수정하지 않고 누른 사용자에게만 임시 응답으로 보낸다.
- [x] 버튼을 누를 때 추가 API를 호출하지 않고 DB 기록을 사용한다.
- [x] 봇 재시작 뒤 버튼, 삭제된 게임 결과, 권한 없는 채널, Discord 전송 실패를 처리한다.

결과: 게임 결과 기능과 기본 채널이 명시적으로 켜진 환경에서만 발송 모니터를 초기화한다. 자동 수집 주기가 끝나면 오래된 `pending` 결과부터 설정된 개수만큼 전송하며, 실패 결과는 다음 주기에 최대 3회까지 재시도한다. 발송 전에 상세 View Model을 경기 DB에 스냅샷으로 저장하므로, 공개 결과의 5개 상세 버튼과 팀 유저 선택 메뉴는 봇 재시작 뒤에도 추가 API 호출 없이 동작한다. 상세 화면은 누른 사용자에게만 임시 응답으로 표시되고 사용자별 선택 상태를 30분 동안 분리해 유지한다. 삭제된 DB 결과, 전송할 수 없는 채널, Discord 오류는 만료 안내 또는 발송 실패 상태로 처리한다.

완료 기준: 자동 결과 메시지는 짧게 유지되고 상세 정보는 누구나 버튼으로 안전하게 확인한다.

### 6. 관리와 운영 설정

- [x] 자동 수집 등록과 게임 결과 알림 등록을 구분한다.
- [x] 관리 도구에 유저별 게임 결과 on/off와 상태 조회를 추가한다.
- [x] 채널 누락·접근 불가·Embed 권한 부족을 시작 시 진단한다.
- [x] 게임 결과 기능이 꺼진 환경에서 모듈 로드·DB 변경·Discord 전송이 없는지 확인한다.
- [x] 엔드필드·KIS 모니터와 오류 및 종료 수명주기를 분리한다.
- [x] 설정·등록·백업·재시작·문제 해결 절차를 README에 추가한다.

결과: 관리 CLI에서 자동 수집 `add/disable/list`와 게임 결과 `receipt on/off/status`를 별도 상태로 관리한다. 게임 결과를 처음 켜면 기존 저장 경기를 `suppressed` 기준선으로 만들며, 자동 수집이 꺼진 유저나 올바른 Discord 채널 ID가 없는 설정은 거부한다. 시작 시 채널 접근과 채널 보기·메시지 보내기·Embed Links 권한을 진단하고, 실패해도 게임 결과 발송만 멈추도록 자동 수집 및 엔드필드·KIS 시작 흐름과 오류 처리를 분리했다. 종료 시 진행 중인 전적 수집과 게임 결과 발송을 각각 기다린 뒤 DB를 닫는다. 기능 비활성 상태의 구독 DB·발송기 미호출과 발송 오류가 수집 주기에 전파되지 않는 동작을 테스트했으며, 설정부터 백업·재시작·복구까지 README에 기록했다.
